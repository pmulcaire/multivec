...
void ReadVocab(struct train_params *params) {
  ...
  SortVocab(params); //sets params->train_words to the sum of the counts of all vocab words
  if (debug_mode > 0) {
    printf("Vocab size: %lld\n", params->vocab_size);
    printf("Words in train file: %lld\n", params->train_words);
  }
...
void *TrainModelThread(void *id) {
  ...
  while (1) {
      ...
      if ((debug_mode > 1)) {
        ...
        if (is_bi){
          printf("%cAlpha: %f, bi_alpha: %f,  Progress: %.2f%%  Words/thread/sec: %.2fk  ", 13, alpha, bi_alpha,
                   (src->word_count_actual)/ (real)(num_threads * src->train_words + 1) * 100,
	...
	break;
      ...
      if (tgt_word_count > tgt->train_words / num_threads) {
	printf("Exceeded target words per thread: %lld / %lld with %d threads\n", tgt_word_count, tgt->train_words, num_threads);
	break;
      }
      // align
    ...
    if (src_word_count > src->train_words / num_threads) {
      printf("Exceeded source words per thread: %lld / %lld with %d threads\n", src_word_count, src->train_words, num_threads);
      break;
    }
  }
...
void MonoInit(struct train_params *params, long long train_words){
  if (access(params->vocab_file, F_OK) != -1) { // vocab file exists
    ...
    if (train_words>0) {
      ...
      CountWordsFromTrainFile(params); //this seems to be totally redundant...
    }
...
int main(int argc, char **argv) {
  ...
  printf("# original output prefix=%s\n", output_prefix);
