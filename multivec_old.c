...
struct pair_params **all_pairs; //malloc'd later
struct lang_params **all_langs; //malloc'd later
...
void *TrainModelThread(void *id) {
  ...
  struct file_params *src_train = pair->src;
  struct file_params *tgt_train = pair->tgt;

  struct lang_params *src_lang = src_train->lang;
  struct lang_params *tgt_lang = tgt_train->lang;
...
#ifdef DEBUG
          printf(" %s", src_lang->vocab[word].word);
#endif
...
all_tgt_words++;
...
#ifdef DEBUG
          printf(" %s", tgt_lang->vocab[word].word); fflush(stdout);
#endif
    ...
    if (feof(tgt_fi)) {
      printf("End of target file\n");
      break;
    }
    if (tgt_word_count > tgt_train->train_words / num_threads) {
      printf("Exceeded target words per thread: %lld / %lld with %d threads\n", tgt_word_count, tgt_train->train_words, num_threads);
      break;
    }
    // align
...
#ifdef DEBUG
    if ((sent_id % 1000) == 0) printf("Done processing sentence\n");
    ...
    if (src_word_count > src_train->train_words / num_threads) {
      printf("Exceeded source words per thread: %lld / %lld with %d threads\n", src_word_count, src_train->train_words, num_threads);
      break;
    }
  }
...
void TrainLanguagePair() {
  long a;
  struct file_params *src = pair->src;
  struct file_params *tgt = pair->tgt;
...
int main(int argc, char **argv) {
  ...
  for(lp1=0; lp1<num_pairs; lp1++) {
    pair = all_pairs[lp1];
    TrainLanguagePair();
    printf("Trained language pair %s-%s\n", pair->src->lang->lang_name, pair->tgt->lang->lang_name);
  }
  return 0;
}
